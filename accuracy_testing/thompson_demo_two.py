...
def plot_results(machine_selection_counts, trial, n_machines, alpha, beta_params, sampled_theta, suggested_machine, cumulative_rewards, optimal_rewards):
    ...
    #Cumulative Rewards vs Optimal Rewards
...
def simulate_with_accuracy_and_regret():
    ...
    n_machines = int(input("Enter the number of machines/arms: "))

    true_conversion_rates = []
    for i in range(n_machines):
        prob = float(input(f"Enter the true probability of success for machine {i}: "))
        true_conversion_rates.append(prob)
